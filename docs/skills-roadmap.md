...
最后更新：2026-06-03（**K-40 完成** = Copilot provider 多轮 function calling 解禁：`CopilotChatClient` 透传 OpenAI 风格 `tools`/`tool_choice` + SSE `tool_calls` 增量按 `index` 拼装；`agent_loop.cpp` 用「默认放行 + 黑名单」策略，除 `o1-mini`/`codex`/老 completion 系外全部启用多轮；实测 `claude-opus-4.7` 跑通 110+ 轮 agent loop。前置 K-39 = 系统侧 5 新工具 76→**80**；K-38 = UTF-8 截断修复；K-37 = retry 误判修复）
...
| **K-40** | **Copilot provider 多轮 function calling 解禁：(1) `CopilotChatClient::streamChat` 透传 OpenAI 风格 `tools`/`tool_choice` 请求拼接 + assistant 携带 `tool_calls` + role=tool 携带 `tool_call_id`，SSE 解 `tool_calls` 增量按 `index` 分组拼装 `id/name/arguments` 分片，非流式一次性解析 + 末尾防御兜底；(2) `agent_loop.cpp` 新增 `modelSupportsToolsViaCopilot(model)` 用「默认放行 + 黑名单」策略，黑名单 = `o1-mini`/`codex`/`embedding`/`embed`/`davinci`/`babbage`/`curie`/`ada`，子串匹配；启动日志 `AgentLoop: provider=N model=X useTools=true/false`；实测 `claude-opus-4.7` 跑通 110+ 轮 read_memory/disasm_at agent loop，`gpt-4o`/`gpt-4.1`/`o3-mini`/`gemini-*` 全部自动放行** | **0** | **0** | — | ✅（未 tag） |

> 工具总数：S8=63 → K-28=64 → K-29=65 → K-31=70 → K-33=74 → K-34=76 → 当前 **80**（K-39 +5 系统侧；K-35-K-38 / K-40 纯编排/bugfix/provider 能力扩展不增工具；详见 `docs/features.md §11 工具清单`）。
